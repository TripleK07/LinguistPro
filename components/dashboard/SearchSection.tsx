...
export const SearchSection: React.FC<SearchSectionProps> = ({ 
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      {isGuest && (
        <div className="bg-white rounded-3xl p-6 border-2 border-indigo-100 shadow-xl shadow-indigo-50/50 flex flex-col sm:flex-row items-center justify-between gap-4 mb-2">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-200">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            </div>
            <div>
              <p className="text-slate-900 font-black text-lg leading-none">Trial Mode Active</p>
              <p className="text-slate-400 text-xs mt-1 font-medium">Experience LinguistPro search features instantly.</p>
            </div>
          </div>
          <button 
            onClick={onExitGuest}
            className="bg-indigo-600 text-white px-8 py-3 rounded-2xl text-sm font-black hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-100"
          >
            Create Full Account
          </button>
        </div>
      ...
      <header className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
        <h1 className="text-2xl font-bold text-slate-800 mb-4 ml-1">Discover New Words</h1>
        <form onSubmit={onSearch} className="flex flex-col sm:flex-row gap-3 items-end">
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-black px-10 py-3.5 rounded-2xl shadow-lg shadow-indigo-100 active:scale-95 disabled:bg-slate-300 transition-all flex items-center justify-center h-[54px] whitespace-nowrap"
          >
