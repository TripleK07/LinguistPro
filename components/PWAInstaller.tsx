...
const PWAInstaller: React.FC = () => {
  ...
  const handleInstallClick = useCallback(async () => {
    if (deferredPrompt) {
      // Trigger the native browser prompt (Chrome, Edge, Android)
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e: any) => {
      // Prevent automatic prompt so we can trigger it with our button
      e.preventDefault();
      setDeferredPrompt(e);
      setIsVisible(true);
    };
    ...
    // If it's iOS and not standalone, we should show the button (it will open the guide)
  ...
  }, [handleInstallClick, detectPlatform, isIOS]);
          ...
          <div className="absolute bottom-full left-0 mb-4 w-72 bg-white rounded-3xl shadow-2xl border border-slate-100 p-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 text-sm text-slate-600">
                ...
                <p>1. Tap the <b>Share</b> icon in the browser toolbar.</p>
              </div>
              <div className="flex items-start gap-4 text-sm text-slate-600">
                <div className="w-8 h-8 bg-indigo-50 rounded-lg flex items-center justify-center flex-shrink-0 text-indigo-600">
                ...
                <p>2. Select <b>'Add to Home Screen'</b> from the menu.</p>
              </div>
            </div>
