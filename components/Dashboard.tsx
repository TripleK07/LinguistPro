            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="space-y-4">
                {data.examples.map((ex, idx) => (
                  <div key={idx} className="border-l-4 border-indigo-600 pl-4 py-1 transition-all hover:bg-slate-50 rounded-r-lg">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm h-fit">
            ...
            <div className="flex flex-wrap gap-2">
              {data.synonyms.map((syn, idx) => (
                <span key={idx} className="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-semibold hover:bg-indigo-50 hover:text-indigo-600 transition-colors cursor-default">{syn}</span>
              ))}
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4">
               {isGuest ? (
                 ...
                 <div className="hidden sm:flex items-center gap-3">
                    <span className="text-xs font-bold text-slate-400 max-w-[150px] truncate">{user.email}</span>
                    <button 
                      onClick={handleSignOut}
                      className="p-2 text-slate-400 hover:text-red-500 transition-colors bg-slate-50 hover:bg-red-50 rounded-lg group"
                      title="Sign Out"
                    >
      ...
      <main className="flex-grow max-w-4xl mx-auto w-full py-8 px-4">
        ...
        {persuasionType && (
          <div className="fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-10 text-center animate-in zoom-in-95 duration-500">
               ...
               <div className="space-y-4">
                 <button 
                   onClick={() => onExitGuest?.()}
                   className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl shadow-xl shadow-indigo-100 transition-all active:scale-95"
                 >
                   Sign up to unlock
                 </button>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">It's free and takes 30 seconds.</p>
                 <button 
